...
| Web-mode media TTL | `data/web/Post.sq` + `WebFeedSource.DEFAULT_MEDIA_TTL_MS` + `refetchForStaleMediaUrl` | t.me/s/ CDN URLs live 1–4 h. Two refresh paths: reactive (Coil load error 401/403/410 → `LocalWebStaleMedia` → `refetchForStaleMediaUrl`, per-URL dedup + per-channel cooldown) and proactive (TTL sweep sets `forceMediaRefresh`, which bypasses the ingest fingerprint guard — deliberately NOT set on a plain pull-to-refresh). |
...
| Interaction-stream contract | `data/posts/PostsRepository.kt:viewMessages` + `data/ChatPresence.kt:isOpen` + `ui/timeline/TimelineScreen.kt` (focus tracker) | TDLib streams `updateMessageInteractionInfo` only for OPEN chats (tdlib/td#2312). Merged feed obeys "usually one chat opened" (tdlib/td#2695) by OpenChat-ing the **dominant-visible** post's chat — NOT `firstVisibleItem`, which on full-width cards is often the partially-clipped neighbour. `viewMessages` picks `force_read` from `ChatPresence.isOpen(chat)`: `false` when open (canonical "user is actively reading"), `true` when closed (closed-chat read-state workaround). The focus tracker must **re-ack the focused post's ids via viewMessages AFTER OpenChat**, in the same `NonCancellable` block — read-ack dwell (500 ms) fires before focus dwell (600 ms), so without the re-ack the focused post's only viewMessages call would have happened with the chat still closed and `force_read=true`. The re-ack (and the Layer-2 album repair) runs for **every** focused post — only the OpenChat/CloseChat swap is gated on a chat transition; a former `focusChat == opened` early-return starved both for consecutive cards of one channel. ChannelScreen doesn't need the re-ack because `ChannelViewModel.init` opens the chat before any viewMessages. |
...
| Rich-message math renderer (jlatexmath) | `ui/rich/RichMath.kt` + `app/proguard-rules.pro` (keep-rule comment) + the `build(rich)` commit body for `f4e7db1` | `ru.noties:jlatexmath-android` — offline pure-Canvas jLaTeXMath port (GPLv2+CE, GPL-3-compatible; ~2.1 MB, upstream archived-but-stable with zero transitive deps). The engine resolves macro/symbol/font handlers REFLECTIVELY from bundled XML — **the `-keep org.scilab.forge.jlatexmath.** / ru.noties.jlatexmath.**` rules are load-bearing**; stripping them crashes release on the first rendered formula. Parse is wrapped in `runCatching` → monospace fallback; never throw to the UI. Don't add a WebView-based math path — feed/reading surfaces must stay WebView-free. |
...
| `LocalMediaCache` / `LocalCustomEmoji` / `LocalExoPlayerPool` / `LocalReadCursors` / `LocalProfileAccent` / `LocalWebStaleMedia` | CompositionLocal heavy-singleton injection (`LocalWebStaleMedia` is the guest-mode stale-media reporter: null in TDLib mode, wired in `WebModeScaffold`). |
...
- **`release.yml`** — tag `v*` (or manual dispatch) → signed AAB + universal APK → GitHub Release (CHANGELOG section as notes). Google Play upload runs when the `PLAY_SERVICE_ACCOUNT_JSON` secret is present (internal track). (FULL native symbols for the AAB are deferred — see the KEEP_DEBUG note below.)
- **`beta.yml`** — after a **green CI run** on `main` (`workflow_run`, so testers never get a build that failed the gate) → signed `dev.lyo.hortay.beta` APK → single rolling `beta` pre-release. Checks out the exact commit CI validated (`workflow_run.head_sha`).
...
- **release/beta MUST build `:libwebm`** — the composite `.github/actions/setup-ffmpeg` installs the NDK and restores (or builds via `scripts/build-ffmpeg.sh`) the gitignored libvpx/ffmpeg static libs that `:libwebm` links into `libhortaywebm.so`. **Never add `-x :libwebm:buildCMakeRelWithDebInfo` to a release/beta build** — that ships an artifact with no WebM(VP9+alpha) decoder and it SIGILLs on the first WebM sticker. (`ci.yml` excludes it deliberately — the lint/test gate never packages the `.so` and the ffmpeg libs aren't built there.) The ffmpeg build (~10-15 min) is cached, keyed on `scripts/build-ffmpeg.sh`, so it reruns only when the recipe changes.
- **TDLib artifacts are gitignored** (`libtdlib/.gitignore`: `*.so`, `TdApi.java`, `Client.java`). The only tracked TDLib state is `scripts/tdlib-version.txt`. Every CI job restores the libs from one `actions/cache` (composite `.github/actions/setup-tdlib`) keyed on the pin **plus the build recipe** (`update-tdlib.sh` + `tdlib-builder/**`, so a recipe change invalidates — not just a version bump), or builds them on a miss. `tdlib.yml` warms this one cache, so after a bump merges ci/release/beta all hit it.
- **CI builds TDLib stripped (`KEEP_DEBUG=0`).** Under AGP 9.3 the two `:libtdlib` jniLibs srcDirs (`src/main/jniLibs` + `build/tdlib-unstripped`) each holding `libtdjni.so` trip `MergeSourceSetFolders` "Duplicate resources" — the old last-srcDir-wins override is gone. So the unstripped overlay is NOT staged in CI, and release AABs currently ship without rich `libtdjni.so` symbolication. **Re-enabling FULL symbols needs the overlay reworked to a single srcDir** (put the unstripped `.so` directly in `src/main/jniLibs`, let AGP strip for packaging + extract symbols) — not a second overlapping srcDir.
