...
- Tapping the reply quote card on a post you've opened (the pinned anchor inside the comments overlay) now actually drills into the original instead of being a dead surface. `CommentsScreen` was building its `pinnedPostInteractions` without an `onQuotedSourceClick` wiring, so the `ReplyBlock` inherited the no-op default and the tap fell on the floor — the only path to the replied-to message was to dismiss the overlay back to the feed and tap the same quote card there. Now mirrors the feed's behaviour: opens the replied-to chat in a fresh `ChannelScreen` overlay with the messageId baked into the entry so the resolver lands at the target and pulses the highlight there; the feed underneath stays put.
