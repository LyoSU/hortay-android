...
### Fixed
- Tap on the reply quote card inside the comments overlay no longer briefly flashes the replied-to post and then snaps back to where the user was before tapping the host post. The overlay was pushing a new `NavEntry.Channel(scrollTo = replyMsgId)` on top of itself — and since the rendering loop only mounts `stack.takeLast(2)`, the originally-displayed `ChannelScreen` for the same channel (sitting at `stack[-3]` after the push) was unmounted, its `NavEntryHost` disposed, and the `openChat → closeChat → openChat` refcount swing in `ChatPresence` produced the visible "throws-to-target, bounces-back" flicker (the user read it as Compose dropping them back to the original scroll position). Comments overlay now pops itself first, then routes the same `safelyOpenChannel(replyChatId, replyMsgId)` push — Telegram-X / Twitter / Reddit's canonical "go to the original" navigation idiom. Back from the freshly-pushed channel returns to the host channel at the user's pre-tap scroll position; the pre-tap `ChannelScreen` stays mounted at `stack[-2]` across the whole transition.
