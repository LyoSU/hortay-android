...
- `OldestUnreadFirst` cold start: redesigned as a *scroll-target* setting (not a sort-order setting). Feed is always rendered in newest-first source order — no mid-streaming re-sort, no LazyColumn key-anchor flicker. Cold start auto-scrolls to the OLDEST unread post (= resume-reading boundary) when there's a clear read-unread mix; otherwise (caught up, all-unread, cursors still loading) the LazyColumn stays at the top (newest), the canonical Twitter-feed landing position. Unread counter pill jumps to the same resume target.
